...
void WhiteWolf::OnCollision(Collider* collider) {
	...
	if (collider->type == Collider::Type::PLAYER_SHOT) {
		...
		App->scene->EnemyCN = 2;
		if (App->player->tranSt == Transform::DEFAULT|| App->player->tranSt == Transform::POWER1|| App->player->tranSt == Transform::POWER2) {
			App->powers->Enable();
		...
		if (App->player->destroyed) {
			if (App->player->tranSt == Transform::POWER1 || App->player->tranSt == Transform::POWER2) {
				App->powers->Disable();
			}
		}
