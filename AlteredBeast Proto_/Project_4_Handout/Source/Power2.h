...
void ModulePlayer::Power2Movement() {
	...
	if (tranSt == Transform::POWER2) {
		...
		if (dir == Direction::RIGHT && airSt == AirState::GROUND) { Pcollider->SetPos(position.x + 40, position.y - 68); }
		if (dir == Direction::LEFT && airSt == AirState::GROUND) { Pcollider->SetPos(position.x + 38, position.y - 68); }
		...
		{
			Pcollider->SetPos(0, position.y - 5000);
			Crouchcollider->SetPos(position.x + 40, position.y - 40); // needs ajustments as soon as the crouch collider is back to funcional
		}
		if (airSt != AirState::CROUCH) { Crouchcollider->SetPos(0, -5000); }

		if (dir == Direction::RIGHT && airSt == AirState::AIRBORN) { Pcollider->SetPos(position.x + 45, position.y - 60); }
		if (dir == Direction::LEFT && airSt == AirState::AIRBORN) { Pcollider->SetPos(position.x + 35, position.y - 60); }
		...
		if (App->input->keys[SDL_SCANCODE_Z] == KEY_DOWN || App->input->pads[0].b) {
			...
			if (idle == true && dir == Direction::RIGHT && airSt == AirState::CROUCH) {
				...
				attackCollider->SetPos(position.x + 66, position.y - 42);
				idle = false;
			}
			if (idle == true && dir == Direction::LEFT && airSt == AirState::CROUCH) {
				AllAnimations.P2CrouchPunchL.Reset();
				currentAnimation = &AllAnimations.P2CrouchPunchL;
				attackCollider->SetPos(position.x + 0, position.y - 42);
				idle = false;
			}
			if (airSt == AirState::AIRBORN) {
				if (dir == Direction::LEFT) {
		...
		if (currentAnimation == &AllAnimations.P2JumpPunchL) {attackCollider->SetPos(position.x + 00, position.y - 60);}
		if (currentAnimation == &AllAnimations.P2JumpKickR)  {attackCollider->SetPos(position.x + 63, position.y - 40);}
		if (currentAnimation == &AllAnimations.P2JumpKickL)  {attackCollider->SetPos(position.x + 01, position.y - 40);}
		...
		if (App->input->keys[SDL_SCANCODE_X] == KEY_DOWN || App->input->pads[0].a) {
			...
			if (idle == true && dir == Direction::RIGHT && airSt == AirState::GROUND) {
				...
				attackCollider->SetPos(position.x + 64, position.y - 30);
				idle = false;
			}
			if (idle == true && dir == Direction::LEFT && airSt == AirState::GROUND)
			{
		...
		if (AllAnimations.P2KickRight.HasFinished() == true) {
			AllAnimations.P2KickRight.loopCount--;   //VERY IMPORTANT , since HasFinished checks if the loop count has surpassed 0, after the animation has finished reset loop count
			idle = true;
		}
		//OUTSIDE THE IF
		if (AllAnimations.P2KickLeft.HasFinished() == true) {
			AllAnimations.P2KickLeft.loopCount--;   //VERY IMPORTANT , since HasFinished checks if the loop count has surpassed 0, after the animation has finished reset loop count
			idle = true;
		}
